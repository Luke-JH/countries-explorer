...
export const useCountryFilters = (countries: Ref<Country[] | null | undefined>) => {
  ...
  const addIncludeFilter = (key: IncludeFilterKey, label: string, value: string) => {
    ...
    if (!trimmed) return;
    includeFilters.value.push({ id: `${key}-${trimmed}-${Date.now()}`, key, label, value: trimmed });
  };
  ...
  const filteredCountries = computed(() => {
    ...
    return list.filter((country) => {
      ...
      if (populationMin.value !== null && country.population < populationMin.value) return false;
      if (populationMax.value !== null && country.population > populationMax.value) return false;
  ...
  const totalPages = computed(() => Math.max(1, Math.ceil(filteredCountries.value.length / PAGE_SIZE)));

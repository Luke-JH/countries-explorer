<template>
  <main class="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    <div v-if="error" class="mt-6 rounded-lg border border-atlas-border bg-atlas-raised p-4 text-sm dark:border-atlas-border-dark dark:bg-atlas-raised-dark" role="alert">
    ...
    <template v-else-if="country">
      <div class="mt-6 flex items-center gap-4">
        ...
        <div>
          ...
          <p class="text-sm text-atlas-muted dark:text-atlas-muted-dark">{{ country.names.official }}</p>
        </div>
      </div>

      <dl class="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3">
        <div>
          <dt class="text-xs font-semibold uppercase tracking-wide text-atlas-muted dark:text-atlas-muted-dark">Region</dt>
          <dd class="mt-1"><RegionTag :region="country.region" /></dd>
        </div>
        <div>
          <dt class="text-xs font-semibold uppercase tracking-wide text-atlas-muted dark:text-atlas-muted-dark">Population</dt>
          <dd class="mt-1 tabular-nums text-atlas-ink dark:text-atlas-ink-dark">{{ formatPopulation(country.population) }}</dd>
        </div>
        <div v-if="capitalNames.length">
          <dt class="text-xs font-semibold uppercase tracking-wide text-atlas-muted dark:text-atlas-muted-dark">Capital</dt>
          <dd class="mt-1 text-atlas-ink dark:text-atlas-ink-dark">{{ capitalNames.join(", ") }}</dd>
        </div>
        <div v-if="currencyNames.length">
          <dt class="text-xs font-semibold uppercase tracking-wide text-atlas-muted dark:text-atlas-muted-dark">Currencies</dt>
          <dd class="mt-1 text-atlas-ink dark:text-atlas-ink-dark">{{ currencyNames.join(", ") }}</dd>
        </div>
        <div v-if="languageNames.length">
          <dt class="text-xs font-semibold uppercase tracking-wide text-atlas-muted dark:text-atlas-muted-dark">Languages</dt>
          <dd class="mt-1 text-atlas-ink dark:text-atlas-ink-dark">{{ languageNames.join(", ") }}</dd>
        </div>
      </dl>
    </template>
  </main>
